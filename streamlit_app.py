...
def analyze_game_with_stockfish(pgn_data, stockfish_path="C:/Users/theso/OneDrive/Desktop/Chess test/stockfish.exe"):
    ...
    try:
        ...
        st.info("Please ensure Stockfish is installed and its path is correct (e.g., `/usr/games/stockfish` on Linux, or specify full path to executable).")
                    ...
                    if top_lines:
                        for line in top_lines:
                            ...
                            try:
                                ...
                                # Temporarily push moves to a new board to get SAN context
                                temp_board = chess.Board(st.session_state.board_states[current_ply -1]) # Board state *before* the move was played
                                san_move = temp_board.san(chess.Move.from_uci(move_uci))
